...
1) Default setup uses the sample docset in `api/docs/minimal` (configured in `api/docsets.toml`), including `api/docs/minimal/weird-python-functions.html`. Docsets can include `.html`/`.htm`, `.md`/`.markdown`, and `.txt` files.
